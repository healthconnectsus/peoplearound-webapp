...
- **`profiles` is column-granted, not just row-scoped** (migration 0050). The update policy from 0001 said "you may edit your own row" and nothing about which fields, and Supabase's default table-wide grant was never narrowed — so `is_admin`, added later in 0025, sat inside a row its owner could write. One line in a browser console with the public anon key promoted any account to operator, which is the whole service-role console: invite by email from our auth domain, read every neighbor's activity, delete any project, and spend the paid provider quota (0040 also exempts admins from every rate cap). Verified end to end, then closed by revoking blanket UPDATE/INSERT from `authenticated` and granting back only the columns a person edits about themselves. The app-layer admin check was never the weak point and could never have been the fix: no server-side verification helps when the client can write the flag the server reads. Column grants fail closed for columns added later, which a trigger pinning one named column would not.
- **Background workers back off when they die** (migration 0051). Both queues leased work but advanced the schedule only in the *finish* write, so any invocation killed mid-run — a `maxDuration` timeout, a deploy — left its row still due. Ten minutes later the same row was claimed again, forever: roughly 864 requests a day at one stranger's website, or a paid search re-spent every ten minutes. The claim now writes the retry time before the work starts, and success pulls it back to the normal cadence. The crawler additionally stops following detail links once its wall-clock budget is spent, so it finishes and saves rather than being killed.
- **HTTP response headers** (`next.config.ts`): the site shipped with only HSTS. Now also `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Permissions-Policy` (camera/microphone/payment off, geolocation kept for "locate me"), and `Referrer-Policy: strict-origin-when-cross-origin`. That last one matters most here: private conversations are addressed as `/chats?c=<uuid>`, and without a policy the full URL travels to every third party the page loads — map tiles, photos — as a `Referer`. Sending only the origin closes that while still giving Mapbox the origin its URL restrictions need.
- **Content Security Policy** (`src/lib/csp.ts`, minted per request in the proxy): nonce-based with `'strict-dynamic'`, not `'unsafe-inline'` — a policy that allows inline script stops approximately nothing, since injecting inline script is the point of XSS. Next stamps the per-request nonce onto every framework and page script automatically; the one hand-written tag (Turnstile) carries it explicitly. `style-src` deliberately keeps `'unsafe-inline'`: a nonce cannot authorise a style *attribute*, and adding one would make browsers ignore `'unsafe-inline'` and silently break every `style={{…}}` in the app. Injected CSS is a far weaker vector than injected script, so it is a trade, not an oversight. **Currently `Content-Security-Policy-Report-Only`** — a wrong policy does not degrade, it blanks the page, so violations are collected first at `/api/csp-report` (size-capped, rate-capped, logs only, always answers 204). Flip to enforcing with `CSP_ENFORCE=1` once reports stay quiet.
- **Endpoints that must answer a stranger.** The proxy redirects any session-less request to `/login`, which silently broke four things that are fetched with no session by something that cannot follow a redirect: `sw.js` (the browser refuses a service worker behind a redirect, so registration failed for every logged-out visitor), `manifest.webmanifest` (so "install this app" never appeared on the one page a new visitor sees), `robots.txt` and `sitemap.xml` (crawlers got a redirect on a site that deliberately publishes city pages). All four are now excluded from the proxy matcher, and `/offline` is an explicit public route — a fallback that redirects is not a fallback.
- **Outbound crawling** (`src/lib/city-events/safe-fetch.ts`) is the only place the server fetches a URL it did not hard-code, so it is treated as hostile: HTTPS only; DNS resolved up front and every returned address checked to be public unicast (blocking loopback, private ranges, link-local and cloud metadata at `169.254.169.254`); the connection then **pinned to the validated IP** with `Host` and TLS servername preserved, which is what defeats DNS rebinding between the check and the request; redirects limited to three and same-origin only, each hop re-validated; 1 MB body cap; 10 s socket timeout inside a 15 s deadline. Discovered calendar sites are stored `enabled = false` and are never fetched until an admin approves them, and `robots.txt` and its crawl-delay are honoured.
- **Cron endpoints** (`/api/digest`, `/api/gardener`, `/api/push`, `/api/import-city-events`, `/api/crawl-city-events`, `/api/welcome-neighbors`) all require `Authorization: Bearer $CRON_SECRET` and return 401 without it — otherwise anyone could trigger mass email, push sends or paid API spend. Every `/admin` server action independently re-verifies `profiles.is_admin` from the session *before* the service-role client is created, so the privileged client cannot be obtained by calling an action directly.
...
- Server-only secrets: `SUPABASE_SERVICE_ROLE_KEY` (admin client in `src/lib/supabase/admin.ts`, guarded by `server-only`), `RESEND_API_KEY` + `ALERT_FROM`/`ALERT_EMAIL` (ops alerts from the verified `peoplearound.com` domain), `UNSPLASH_ACCESS_KEY` (free tier, 50 API calls/hour — the limit counts *calls*, not photos, so we fetch Unsplash's per-page maximum of 30 in one request and cache them; `scripts/warm-stock-photos.mjs` pre-warms all 13 category searches so no neighbor waits on a live call. Only metadata is stored — the images stay hotlinked to Unsplash's CDN, which their API guidelines require. — the wizard's cover-photo step offers 3 real, keyword-searched stock photos alongside upload; `/api/unsplash-photos` and its `/track` download-tracking ping, required by Unsplash's API guidelines whenever a returned photo is actually used, keep the key server-side; unset means the picker hides itself rather than erroring).
...
- **Per-user abuse caps** (migration 0017, DB-trigger enforced so accounts can't route around them): projects 10/day · communities 3/day (+`created_by` attribution) · conversations 20/day · messages 200/hour · AI idea-shaping 20/day (`consume_ai_credit()`; the shape-idea route returns 429 past the cap — every call costs Claude API money). Operator SQL and service-role paths are exempt — and since 0040, so are **admins**: `assert_rate` returns early for `profiles.is_admin`, which covers every capped table at once (they all funnel through it) plus any cap added later. `consume_ai_credit` and the 0038 door check carry the same exemption so enforcement and the UI never disagree. Admin actions are not logged at all, so seeding a demo doesn't fill the operator's own history.
