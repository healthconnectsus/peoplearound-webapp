...
- **Multiple communities** — a user belongs to many communities (kinds: neighborhood, cultural, hobby, identity, geographic, interest), managed at [`/neighborhood`](src/app/neighborhood/page.tsx): join, leave, create, and set a primary. The home feed's "In your communities" zone spans all of them; the profile's `neighborhood_id` remains the primary. Requires migration `0011` (`community_members` + `neighborhoods.kind`).
- **Messaging** — Nextdoor-style chats at [`/chats`](src/app/chats/page.tsx): conversation list with unread dots, 1:1 threads, new-message picker, "Message" buttons on People/Connections, live updates via Supabase Realtime. Tables `conversations` / `conversation_participants` / `messages` with participant-scoped RLS (migration `0011`).
