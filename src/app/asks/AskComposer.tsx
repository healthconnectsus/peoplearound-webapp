  ...
  return (
    ...
    >
      ...
      <div className="mt-2 flex flex-wrap gap-1.5">
        {EXAMPLES.map((e) => (
          <button
            ...
            type="button"
            onClick={() => setTitle(e)}
            className="rounded-full border border-slate-400 px-3 py-1 text-xs text-black/55 transition-colors hover:border-emerald-600 hover:text-emerald-700 dark:border-slate-500 dark:text-white/55 dark:hover:text-emerald-400"
          >
            {e}
          </button>
        ))}
      ...
      <input
        ...
        className="mt-4 w-full rounded-xl border border-slate-400 bg-transparent px-3.5 py-2.5 text-sm outline-none focus:border-emerald-600 dark:border-slate-400"
      />
      <textarea
        name="description"
        rows={2}
